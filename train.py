...
def train(model: GPT, data: torch.utils.data.Dataset, config: TrainConfig):
    ...
    assert config.batch_size % (config.minibatch_size * model.config.context_len * DDP_WORLD_SIZE) == 0, "batch size should be divisible by (minibatch_size * context_len * DDP_WORLD_SIZE)"
    grad_accum_steps = config.batch_size // (config.minibatch_size * model.config.context_len * DDP_WORLD_SIZE)
